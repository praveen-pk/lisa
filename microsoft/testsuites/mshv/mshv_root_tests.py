...
from lisa.tools import Service
...
class MshvHostTestSuite(TestSuite):
    ...
    def before_case(self, log: Logger, **kwargs: Any) -> None:
        ...
        if not node.tools[Ls].path_exists("/dev/mshv_diag", sudo=True):
            raise SkippedException("mshv_diag module should be loaded on MSHV"
                                   " root parition.")
    ...
    def mshvlog_is_active(
        ...
        if not mshvlog_running:
            log.error(f"mshvlog service is not running on MSHV root partition.")
        ...
        if mshvdiag_dmesg_logs is not None:
            log.error(f"mshv_diag module reported errors in dmesg: "
