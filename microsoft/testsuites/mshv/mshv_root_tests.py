...
class MshvHostTestSuite(TestSuite):
    ...
    @TestCaseMetadata(
        description="""
        Ensure mshvtrace tool is present, can be executed, and produces output on the MSHV root partition.
        The test checks:
    ...
    def verify_mshvtrace_tool(
        ...
        temp_dir = node.execute("mktemp -d /tmp/mshvtrace_test_XXXXXX", sudo=True).stdout.strip()
        ...
        try:
            ...
            is_executable = node.execute(f"test -x {mshvtrace_path}", sudo=True).exit_code == 0
            assert_that(is_executable).described_as("mshvtrace should be executable").is_true()
            ...
            _ = node.execute(f"{mshvtrace_path} collect -o {trace_output}", sudo=True, timeout=35)
            trace_size = node.execute(f"stat -c %s {trace_output}", sudo=True).stdout.strip()
            # 8192 is the min size of the trace file.
            assert_that(int(trace_size)).described_as("mshvtrace output should be greater than 8192 bytes").is_greater_than(8192)
